...
import { Flame, ShoppingBag, TrendingUp, LogOut, User } from "lucide-react";
...
// Correo(s) y/o nombre(s) de usuario que tienen acceso al panel de edición del catálogo
const ADMIN_EMAILS = ["darienapm@icloud.com"];
const ADMIN_NAMES = ["drrn.pm", "drrn"];
...
function Store({ user, onLogout }) {
  ...
  const isAdmin =
    ...
    ADMIN_NAMES.includes((user.name || "").toLowerCase());
  ...
  const handleBuy = async (cap) => {
    // Optimista: refleja el cambio de inmediato en la UI
    setCaps((prev) => prev.map((c) => (c.id === cap.id ? { ...c, sold: c.sold + 1 } : c)));
    setFeed((prev) =>
      [
        {
          id: `local-${Date.now()}`,
          buyer: user.name,
          edition: `${cap.name} · ${cap.edition}`,
          time: "ahora",
          isYou: true,
        },
        ...prev,
    ...
    setToast(`Agregaste ${cap.name} — ${cap.edition}`);
    setTimeout(() => setToast(null), 2200);

    try {
      await sbPost("purchases", { buyer_name: user.name, cap_id: cap.id });
      await sbPatch(`caps?id=eq.${cap.id}`, { sold: cap.sold + 1 });
      // Re-sincroniza con la fuente real
      loadFromSupabase();
    } catch (e) {
      setToast("No se pudo registrar la compra en Supabase");
          ...
          <ol style={styles.rankList}>
            {ranked.map((cap, i) => (
              <li
                ...
                style={{
                  ...styles.rankItem,
                  boxShadow: flashId === cap.id ? "0 0 0 1px #D4AF37, 0 0 24px rgba(212,175,55,0.35)" : "none",
                }}
              >
            ...
            {caps.map((cap) => (
              <div key={cap.id} style={styles.card}>
                ...
                <div style={styles.cardBody}>
                  ...
                  <button style={styles.buyBtn} onClick={() => handleBuy(cap)}>
                    Comprar
                  </button>
                </div>
              </div>
...
const styles = {
  page: { minHeight: "100vh", background: "#0B0B0D", color: "#F5F1E8", fontFamily: "'Inter', sans-serif" },
  ticker: {
